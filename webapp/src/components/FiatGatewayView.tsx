...
export default function FiatGatewayView() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      setBuyResult(`Achat de ${tx.warpAmount.toFixed(2)} ⬣ pour ${formatFiatPrice(buyFiat, currency)}`);
      setBuyAmount('');
      setTransactions(gateway.getTransactionsByAddress(wallet.address));
    } catch (e: any) {
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      await send('FIAT_GATEWAY', sellWarpAmount, `Fiat sell: ${sellWarpAmount} ⬣`);
      setSellResult(`Vente de ${sellWarpAmount} ⬣ pour ${formatFiatPrice(tx.sellerReceives, currency)} (après frais)`);
      setSellWarps('');
      setTransactions(gateway.getTransactionsByAddress(wallet.address));
    } catch (e: any) {
    ...
    <div className="space-y-4 pb-8">
      ...
      {tab === 'buy' && (
        <div className="glass-panel p-5 space-y-4">
          ...
          {buyFiat > 0 && (
            <div className="bg-current/5 p-3 space-y-1 text-body-sm">
              <div className="flex justify-between">
                ...
                <span className="font-bold opacity-90">{buyWarps.toFixed(2)} {'⬣'}</span>
              </div>
              <div className="flex justify-between">
                <span className="opacity-50">Taux</span>
                <span className="opacity-60">1 {currency} = {gateway.getRate(currency)?.warpsPerUnit} {'⬣'}</span>
              </div>
              {buyFees && (
                <>
                  <div className="flex justify-between">
          ...
          >
            {buying ? 'En cours...' : `Acheter ${buyWarps > 0 ? buyWarps.toFixed(2) + ' ⬣' : 'Strangrz'}`}
          </button>
      ...
      {tab === 'sell' && (
        <div className="glass-panel p-5 space-y-4">
          ...
          >
            {selling ? 'En cours...' : `Vendre ${sellWarpAmount > 0 ? sellWarpAmount.toFixed(2) + ' ⬣' : 'Strangrz'}`}
          </button>
